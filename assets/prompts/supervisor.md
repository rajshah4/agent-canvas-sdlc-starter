...
Leave Agent Canvas agent profile selection blank unless the user explicitly asks for a specific agent profile. Use the active OpenHands LLM profile/model settings and the required terminal, file, and task-tracking tools. If the user asks why a model was selected, explain the difference between the agent profile picker and the LLM profile/model setting.
